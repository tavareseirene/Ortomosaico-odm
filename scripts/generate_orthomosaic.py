...
INPUT_IMAGES_DIR = Path(
    "/home/savefarm/Documents/Estudo Crop/ortomosaico/imagens/ortomosaico-80mm-full"
)
...
# Mantendo CPU porque sua versão GPU deu erro de CUDA/driver.
ODM_DOCKER_IMAGE = "opendronemap/odm"

# Resolução do ortomosaico em cm/pixel.
# 1.0 = 1 cm/pixel
# 2.0 = 2 cm/pixel, mais leve
...
# Para ortomosaico agrícola, normalmente não precisa do modelo 3D final.
SKIP_3D_MODEL = True

# Mais rápido, mas pode perder qualidade porque pula reconstrução densa.
FAST_ORTHOPHOTO = False

# Cria Cloud Optimized GeoTIFF e overviews
CREATE_COG = True
...
# 16 é mais seguro para evitar erro 137 por falta de RAM.
MAX_CONCURRENCY = 8

# Continuar a partir da etapa que falhou.
# Para rodar do zero, use: RERUN_FROM = None
RERUN_FROM = None

# Pula correção local de emendas da textura.
# Isso reduz uso de RAM exatamente na etapa onde seu processo morreu.
TEXTURING_SKIP_LOCAL_SEAM_LEVELING = False

# Se ainda morrer por RAM, você pode colocar True também.
TEXTURING_SKIP_GLOBAL_SEAM_LEVELING = False

# Usar hardlink quando possível para não duplicar imagem
USE_HARDLINK_OR_COPY = True

# IMPORTANTE:
# Como você quer continuar o projeto já processado, deixe False.
# Se colocar True, apaga imagens ou processamento anterior.
...
def prepare_odm_project(images):
    ...
    return project_dir, images_dir, copied, hardlinked, copied_real, already_exists
...
def pull_odm_image():
    ...
    subprocess.run(["docker", "pull", ODM_DOCKER_IMAGE], check=True)
...
def build_odm_command():
    ...
    if RERUN_FROM is not None:
        cmd.extend(["--rerun-from", RERUN_FROM])
    ...
    if TEXTURING_SKIP_LOCAL_SEAM_LEVELING:
        cmd.append("--texturing-skip-local-seam-leveling")

    if TEXTURING_SKIP_GLOBAL_SEAM_LEVELING:
        cmd.append("--texturing-skip-global-seam-leveling")
...
def copy_final_outputs(project_dir: Path):
    ...
    # Copia PNG/KMZ/TFW se existirem
    ortho_dir = project_dir / "odm_orthophoto"
    ...
    # Copia relatório se existir
    report_pdf = project_dir / "odm_report" / "report.pdf"
    if report_pdf.exists():
        dst = FINAL_OUTPUT_DIR / f"{PROJECT_NAME}_report.pdf"
        shutil.copy2(report_pdf, dst)
        print(f"[OK] Relatório copiado: {dst}")
...
def print_disk_usage(project_dir: Path):
    print("\n[INFO] Espaço livre no disco do projeto:")
    subprocess.run(["df", "-h", str(PROJECTS_ROOT)], check=False)

    if project_dir.exists():
        print("\n[INFO] Tamanho atual do projeto ODM:")
        subprocess.run(
            ["sudo", "du", "-h", "--max-depth=1", str(project_dir)],
            check=False
        )
...
def main():
    ...
    project_dir, images_dir, copied, hardlinked, copied_real, already_exists = prepare_odm_project(images)

    print("\n[INFO] Projeto ODM preparado:")
    print(f"Projeto: {project_dir}")
    print(f"Imagens: {images_dir}")
    print(f"Novas imagens adicionadas: {copied}")
    print(f"Hardlinks criados: {hardlinked}")
    print(f"Cópias reais feitas: {copied_real}")
    print(f"Imagens já existentes: {already_exists}")

    print_disk_usage(project_dir)
